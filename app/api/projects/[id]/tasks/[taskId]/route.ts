...
export async function PATCH(
  ...
  if (body.status) {
    Sentry.logger.info(
      Sentry.logger.fmt`Task status changed: ${updated.title} -> ${updated.status}`,
      { taskId: updated.id, projectId: updated.projectId, status: updated.status, priority: updated.priority }
    );
